...
AIの処理はサーバー側で行っており、送信されるのは会話の内容（直近の一定件数のみ）と、駅の候補を絞り込むための乗車中の駅の情報、表示言語の設定です。位置情報そのものは送信されません。送信された会話の内容をTinyKittenのサーバーに保管することはありません。また、端末にも保存されず、画面を閉じると破棄されます。
